...
def get_file_sizes(path):
    ...
    for dirName, subdirs, fileList in os.walk(path):
        ...
        for filename in fileList:
            ...
            print(f"🕵🏼‍♂️  Checking file size on: {file_path}.. ", end="")
            try:
                file_size = os.path.getsize(file_path)
                print(file_size)
                file_sizes.append([file_path, file_size])
            except OSError:
                print(f"Unable to get file size ({OSError})")
...
def check_dupe_file_sizes():
    ...
    if dupes_array:
        print("👯  Found %s files with identical file sizes:" % (len(dupes_array)))
        print(
            pandas.DataFrame(
                data=dupes_array, columns=["Filename", "Size"]
            ).sort_values(by=["Size"])
        )
...
def get_file_hashes(file_path):
    ...
    print(f"🤖  Getting file hash for: {file_path}.. ", end="")
    try:
...
def print_results():
    ...
    if len(dupe_files_array) == 0:
        ...
        print(
            pandas.DataFrame(
                data=sorted(dupe_files_array, key=lambda x: x[1]),
        ...
        for f in dupe_files_array:
            ...
            if f[1] in seen_hashes:
                ...
                selected_file = questionary.select(
                    "Which file would you like to delete?", choices=files_to_delete
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="A tool for checking for duplicate files."
    )
    parser.add_argument(
        "-p",
        "--path",
    ...
    if scanlist:
        print(
            "🐶  Checking for duplicate files (with extensions %s) in %s"
            % (scanlist, scanpath[0])
        )
